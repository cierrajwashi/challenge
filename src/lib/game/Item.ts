...
class Item implements GameBoardItem {
  ...
  setPiece(
    piece: GameBoardPiece,
    direction: GameDirection = GameDirection.NONE
  ): void {
    this.piece = piece;
  ...
  fillBackgroundItem(): void {
    if (this.backgroundItem !== false) {
      ...
      this.items[this.piece.y][this.piece.x] = {
        type: GameBoardItemType.EMPTY,
      };
    }
  ...
  findItem(
    ...
    while (
      ...
      currentPiece.type !== GameBoardPieceType.WALL
    ) {
   ...
   * @param {string} directionKey Direction to look in
   * @param {GameBoardItemType} typeToFind Type of item being looked for
   * @return {number | false} Distance to item found
   */
  findItemDistance(
    directionKey: string,
    typeToFind: GameBoardItemType
